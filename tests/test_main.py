...
from services.math_service import fibonacci_n, MAX_ABSOLUTE_VALUE, pow_int, factorial
from exceptions.exceptions import OverflowErr
...
async def test_pow_invalid_payload_returns_422(session_override, payload):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert isinstance(body["detail"], list)            # list of error items
...
def test_pow_int_caching(monkeypatch):
    ...
    monkeypatch.setattr(math_service, "pow_int", functools.lru_cache(maxsize=256)(counted_pow))
...
def test_fibonacci_n_caching(monkeypatch):
    ...
    monkeypatch.setattr(math_service, "fibonacci_n", functools.lru_cache(maxsize=1024)(counted_fib))
...
def test_factorial_caching(monkeypatch):
    ...
    monkeypatch.setattr(math_service, "factorial", functools.lru_cache(maxsize=512)(counted_factorial))
